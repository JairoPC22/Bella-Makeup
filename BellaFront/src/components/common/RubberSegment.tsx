// Adaptado de React Bits (reactbits.dev). Se usa en filtros de opción única
// (ej. "Hoy / Ayer / Últimos 30 días"): agrega un thumb animado que se
// desliza y estira entre opciones en vez de solo cambiar el color de fondo.
...
// Wrapper para un falso error de tipos de `animate()` con MotionValue<number>
// tras una actualización de dependencia (no es un error real: `trail`, con
// los mismos valores, sí resuelve bien). Evita castear en cada call site.
function animateNumber(value: MotionValue<number>, target: number, options: Record<string, unknown>) {
  return (animate as (v: MotionValue<number>, t: number, o: Record<string, unknown>) => ReturnType<typeof animate>)(value, target, options);
}
...
export default function RubberSegment({
  ...
  const list: RubberSegmentItem[] = items.map((item) => (typeof item === "string" ? { value: item, label: item } : item));
  ...
  const travel = (from: number, to: number) => {
    ...
    animateNumber(edgeL, b.l + (Math.min(a.l, b.l) - b.l) * u, tween);
    animateNumber(edgeR, b.r + (Math.max(a.r, b.r) - b.r) * u, tween);
