// Adaptado de React Bits (reactbits.dev). Se usa en confirmaciones
// destructivas (eliminar rol, etc.) en vez de un botón "Sí, eliminar":
// mantener presionado es mucho más difícil de disparar por accidente.
import { useEffect, useId, useLayoutEffect, useRef, useState, type CSSProperties, type ReactNode, type PointerEvent as ReactPointerEvent, type KeyboardEvent as ReactKeyboardEvent } from "react";
...
export default function HoldButton({
  ...
  const handlePointerMove = (e: ReactPointerEvent<HTMLButtonElement>) => {
    ...
    if (!r) return;
    const out =
      e.clientX < r.left - HIT_PAD || e.clientX > r.right + HIT_PAD || e.clientY < r.top - HIT_PAD || e.clientY > r.bottom + HIT_PAD;
  ...
  useEffect(() => {
    ...
    const m = motion.current;
    return () => {
      clearTimeout(timersNow.complete);
      clearTimeout(timersNow.reset);
      cancelAnimationFrame(m.raf);
    };
  }, []);
  ...
  return (
    ...
    >
      ...
      <span id={hintId} className="hold-button__sr">
        Presiona y mantén por {Math.round(holdTime / 100) / 10} segundos para confirmar
      </span>
    </button>
  );
}
